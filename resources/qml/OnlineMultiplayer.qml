...
Item {
	...
	TetroClientQ {
		...
		onChallengeSent: { console.log("Challenge Sent!") }
		onChallengeAlert: { lobby.activateDialog(username + " has challenged you.") }
		onDisconnected: { console.log("Opponent disconnected") }
	...
	Row {
			...
			PlayArea {
				...
				onGetPoints: { client.sendRemovePoints(num_points) }
				onSendSpecial: { client.sendGetSpecial(special_type) }
				onGameFailed: { client.sendWinGame(); gameOverMenu.appear("Right-Side Wins!") }
